...
const getOffenseXY = (player, layout) => {
  ...
  if (!STARTERS.offense.includes(id) && !STARTERS[baseKey].includes(id)) {
    return [null, null];
  }
...
const getDefenseXY = (player, layout) => {
  ...
  if (!STARTERS.defense.includes(id) && !STARTERS[baseKey].includes(id)) {
    if ( id.includes('1')) console.log('need to add', id);
    return [null, null];
  }
